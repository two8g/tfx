...
class NotebookFormatter(object):
  ...
  def _extended_getattr(self, obj: object, property_name: Text):
    """Get a possibly nested attrute of a given object."""
  ...
  def render(self,
             obj: object,
             expanded: bool = True,
             seen_elements: Optional[set] = None):  # pylint: disable=g-bare-generic
  ...
  def render_value(self,
                   value: object,
                   seen_elements: set):  # pylint: disable=g-bare-generic
...
FORMATTER_REGISTRY = _create_formatters({
    Artifact: {
        ...
        'title_format': (
            ('<span class="class-name">%s</span> of type '
    ...
    Channel: {
        ...
        'title_format': (
            ...
            [
                '__class__.__name__', 'type_name', lambda o: len(o._artifacts),  # pylint: disable=protected-access
...
def register_formatters():
  from IPython.core.getipython import get_ipython  # pylint: disable=g-import-not-at-top
  ipython = get_ipython()
  if ipython:
