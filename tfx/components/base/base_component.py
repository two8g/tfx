...
class ExecutorSpec(object):
  """A specification of component executor.

  An instance of ExecutionSpec describes the implemenation of a component.
...
class BaseComponent(with_metaclass(abc.ABCMeta, object)):
  ...
  Attributes:
    ...
    SPEC_CLASS: a subclass of types.ComponentSpec used by this component
      (required).
    EXECUTOR_SPEC: an instance of ExecutorSpec which describes how to execute
      this component (required).
  ...
  def __init__(self,
               ...
               custom_executor_spec: Optional[Type[ExecutorSpec]] = None,
    ...
    if custom_executor_spec:
      if not isinstance(custom_executor_spec, ExecutorSpec):
        raise TypeError(
  ...
  def _validate_component_class(cls):
    ...
    if not isinstance(cls.EXECUTOR_SPEC, ExecutorSpec):
      raise TypeError((
